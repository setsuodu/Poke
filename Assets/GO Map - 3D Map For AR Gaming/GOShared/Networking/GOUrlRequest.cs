			...
			if (Application.isPlaying) { //Runtime build
				...
				else
				{
					...
					if (www.error == null && www.bytes.Length > 0) {
						...
						Debug.LogWarning("[GO TERRAIN] Tile data missing "+www.error + " " + url);
				...
				else
				{
					...
					ContinuationManager.Add(() => www.isDone, () => {
						...
						if (www.error == null && www.bytes.Length > 0) {
							...
							Debug.LogWarning("[GOUrlRequestN] data reload "+www.error);
							System.Threading.Thread.Sleep(1000);
							GORoutine.start(getRequest(host,url,useCache,filename,response),host);
						}
						else {
							Debug.LogWarning("[GO TERRAIN] Tile data missing "+www.error);
							response(null,null,www.error);
						}
					});
			#endif
